...
std::vector<int> matching_FatJet(cRVecF FatJet_eta, cRVecF FatJet_phi, cRVecF GenVisTau_eta, cRVecF GenVisTau_phi){
    
    std::vector<int> output_FaJet;

    std::vector<int> dr_taujet_lessthan8;

    std::vector<int> just_one_match;
    std::vector<int> zero_match;

    for(auto i = 0; i < FatJet_eta.size(); i++){
        if (deltaR(FatJet_eta[i],GenVisTau_eta[0],FatJet_phi[i],GenVisTau_phi[0])<0.8){
            if (deltaR(FatJet_eta[i],GenVisTau_eta[1],FatJet_phi[i],GenVisTau_phi[1])<0.8){
                dr_taujet_lessthan8.push_back(i);
            }
            else{
                just_one_match.push_back(i);
            }
        }
        else{
            if (deltaR(FatJet_eta[i],GenVisTau_eta[1],FatJet_phi[i],GenVisTau_phi[1])<0.8){
...
std::vector<int> matching_FatJet_number(cRVecF FatJet_eta, cRVecF FatJet_phi, cRVecF GenVisTau_eta, cRVecF GenVisTau_phi){
    ...
    output_FaJet.push_back(dr_taujet_lessthan8.size());
    output_FaJet.push_back(just_one_match.size());
    output_FaJet.push_back(zero_match.size());
    return output_FaJet;
}


int matching_tau_Gentau(cRVecF GenVisTau_eta, cRVecF GenVisTau_phi, std::vector<int> vec, cRVecF tau_eta, cRVecF tau_phi, float dr){
    int AB_match = 0;
    int BA_match = 0;
    
    if (deltaR(tau_eta[vec[1]],GenVisTau_eta[0],tau_phi[vec[1]],GenVisTau_phi[0])<dr && deltaR(tau_eta[vec[2]],GenVisTau_eta[1],tau_phi[vec[2]],GenVisTau_phi[1])<dr){
        AB_match = 1;
    }
    if (deltaR(tau_eta[vec[1]],GenVisTau_eta[1],tau_phi[vec[1]],GenVisTau_phi[1])<dr && deltaR(tau_eta[vec[2]],GenVisTau_eta[0],tau_phi[vec[2]],GenVisTau_phi[0])<dr){
        BA_match = 1;
    }

    return AB_match + BA_match;
}

int matching_bt_sj(std::vector<int> vec0, cRVecF tau_eta0, cRVecF tau_phi0, std::vector<int> vec, cRVecF tau_eta, cRVecF tau_phi, float dr){
    int AB_match = 0;
    int BA_match = 0;
    
    if (deltaR(tau_eta[vec[1]],tau_eta0[vec0[1]],tau_phi[vec[1]],tau_phi0[vec0[1]])<dr && deltaR(tau_eta[vec[2]],tau_eta0[vec0[2]],tau_phi[vec[2]],tau_phi0[vec0[2]])<dr){
        AB_match = 1;
    }
    if (deltaR(tau_eta[vec[1]],tau_eta0[vec0[2]],tau_phi[vec[1]],tau_phi0[vec0[2]])<dr && deltaR(tau_eta[vec[2]],tau_eta0[vec0[1]],tau_phi[vec[2]],tau_phi0[vec0[1]])<dr){
        BA_match = 1;
    }
...
float M_tautau_CA_boostedtau_SubJet(cRVecF boostedTau_eta, cRVecF boostedTau_phi, cRVecF boostedTau_mass, cRVecF boostedTau_pt,
                    cRVecF SubJet_eta, cRVecF SubJet_phi, cRVecF SubJet_mass, cRVecF SubJet_pt, 
                    std::vector<int> vec_boostedtau, std::vector<int> vec_SubJet,
                    float GenMET_pt,float GenMET_phi,
                    float mass_vistautau){
    
    float M_tautau_CA_boostedtau_SubJet = 0;

    if(vec_boostedtau[0] >1 ){
        int tau0_id = vec_boostedtau[1];
        int tau1_id = vec_boostedtau[2];
        if(mass_vistautau >-1){
            M_tautau_CA_boostedtau_SubJet = M_tautau_CA(boostedTau_eta, boostedTau_phi, 
                        boostedTau_mass, boostedTau_pt, 
                        tau0_id, tau1_id,
            ...
            ROOT::Math::PtEtaPhiMVector v4_tau0(boostedTau_pt[tau0_id], 
                       boostedTau_eta[tau0_id], 
                       boostedTau_phi[tau0_id], 
                       boostedTau_mass[tau0_id]);
    
            ROOT::Math::PtEtaPhiMVector v4_tau1(boostedTau_pt[tau1_id], 
                       boostedTau_eta[tau1_id], 
                       boostedTau_phi[tau1_id], 
                       boostedTau_mass[tau1_id]);

            ROOT::Math::PtEtaPhiMVector v4_tautau = v4_tau0+v4_tau1;

            //std::cout<<"v4_tautau.M() bt "<<v4_tautau.M()<<endl;
            //std::cout<<"tau1+tau2 bt "<<boostedTau_mass[tau0_id]+boostedTau_mass[tau1_id]<<endl;
        ...
        if(vec_SubJet[0] > 1){
            int tau0_id = vec_SubJet[1];
            int tau1_id = vec_SubJet[2];
            if(mass_vistautau >-1){
                M_tautau_CA_boostedtau_SubJet = M_tautau_CA(SubJet_eta, SubJet_phi, 
                        SubJet_mass, SubJet_pt, 
                        tau0_id, tau1_id,
                        GenMET_pt,GenMET_phi,
                        mass_vistautau);
            }
                ...
                ROOT::Math::PtEtaPhiMVector v4_tau0(SubJet_pt[tau0_id], 
                       SubJet_eta[tau0_id], 
                       SubJet_phi[tau0_id], 
                       SubJet_mass[tau0_id]);
    
                ROOT::Math::PtEtaPhiMVector v4_tau1(SubJet_pt[tau1_id], 
                       SubJet_eta[tau1_id], 
                       SubJet_phi[tau1_id], 
                       SubJet_mass[tau1_id]);
                ...
                //std::cout<<"mass_vistautau sj "<<mass_vistautau<<endl;
                //std::cout<<"v4_tautau.M() sj "<<v4_tautau.M()<<endl;
                //std::cout<<"tau1+tau2 sj "<<SubJet_mass[tau0_id]+SubJet_mass[tau1_id]<<endl;
    ...
    return M_tautau_CA_boostedtau_SubJet;
